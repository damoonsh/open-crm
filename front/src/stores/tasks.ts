...
export const useTaskStore = defineStore("tasks", {
  ...
  actions: {
    ...
    async deleteTask(taskId: number): Promise<boolean> {
      ...
      const mutation = `
        mutation DeleteTask($id: Int!, $requestor_username: String!) {
          deleteTask(id: $id, requestor_username: $requestor_username)
        }
      `;
      const variables = { id: parseInt(taskId), requestor_username: authStore.username };
    ...
    async updateTaskStatus(taskId: number, status: string): Promise<Task | null> {
      const authStore = useAuthStore();
      this.loading = true;
      this.error = null;
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.detail || `Failed to update task status: ${response.statusText}`);
        }
    ...
    async updateTaskCategory(taskId: number, categoryId: number | null): Promise<Task | null> {
      const authStore = useAuthStore();
      this.loading = true;
      this.error = null;
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.detail || `Failed to update task category: ${response.statusText}`);
        }
        ...
        this.error = error.message || 'Failed to update task category';
    ...
    async moveTaskToWorkspace(taskId: number, workspaceId: number): Promise<Task | null> {
      const authStore = useAuthStore();
      this.loading = true;
      this.error = null;
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.detail || `Failed to move task to workspace: ${response.statusText}`);
        }
        ...
        this.error = error.message || 'Failed to move task to workspace';
        console.error('Error moving task to workspace:', error);
        return null;
      } finally {
        this.loading = false;
      }
    ...
    async getTaskDependencies(taskId: number) {
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.detail || `Failed to fetch task dependencies: ${response.statusText}`);
        }

        const dependencies = await response.json();
        return dependencies;
      } catch (error: any) {
        this.error = error.message || 'Failed to fetch task dependencies';
    ...
    async addTaskDependency(taskId: number, blockedTaskId: number, dependencyType: string = 'blocks') {
      const authStore = useAuthStore();
      this.loading = true;
      this.error = null;
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.detail || `Failed to add task dependency: ${response.statusText}`);
        }

        const dependency = await response.json();
        return dependency;
      } catch (error: any) {
        this.error = error.message || 'Failed to add task dependency';
    ...
    async removeTaskDependency(taskId: number, blockingTaskId: number) {
      const authStore = useAuthStore();
      this.loading = true;
      this.error = null;

      try {
        const response = await fetch(`/api/tasks/${taskId}/dependencies/${blockingTaskId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${authStore.token}`,
            'Content-Type': 'application/json',
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.detail || `Failed to remove task dependency: ${response.statusText}`);
        }

        return true;
      } catch (error: any) {
        this.error = error.message || 'Failed to remove task dependency';
    ...
    async validateDependency(blockingTaskId: number, blockedTaskId: number) {
      const authStore = useAuthStore();
      this.loading = true;
      this.error = null;

      try {
        const response = await fetch('/api/tasks/dependencies/validate', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${authStore.token}`,
            'Content-Type': 'application/json',
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.detail || `Failed to validate dependency: ${response.statusText}`);
        }

        const validation = await response.json();
        return validation;
      } catch (error: any) {
        this.error = error.message || 'Failed to validate dependency';
        console.error('Error validating dependency:', error);
        return { is_valid: false, reason: error.message };
      } finally {
        this.loading = false;
      }
    },

    async fetchWorkspaceTasks(workspaceId: number) {
      const authStore = useAuthStore();
      this.loading = true;
      this.error = null;
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.detail || `Failed to fetch workspace tasks: ${response.statusText}`);
        }
        ...
        this.error = error.message || 'Failed to fetch workspace tasks';
